...
export class DistributeSolController {
	...
	async distributeSolToHolder(data: PrepareRewardDistributionPayload) {
		...
		const totalTax = token.rewardTax + token.jackpotTax
		...
		if (token.lockAmount) {
			...
			if (creatorIndex === -1) {
				...
				holderData.holder[creatorIndex].amount =
					holderData.holder[creatorIndex].amount + token.lockAmount
			}
		...
		for (let i = 0; i < holderData.holder.length; i += batchSize) {
			...
			for (const holder of batch) {
				const amountHolder = BigInt(holder.amount)
